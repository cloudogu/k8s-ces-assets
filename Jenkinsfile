            ...
            stage('Update development resources') {
                ...
                docker.image("golang:${goVersion}")
                        ...
                        .inside("--volume ${WORKSPACE}:/workdir -w /workdir") {
                            sh "STAGE=development IMAGE_DEV=${repository} IMAGE_DEV_WARP=${repository}-warp IMAGE_DEV_MAINTENANCE=${repository}-maintenance make helm-values-replace-image-repo"
                        }
            }
...
void stageAutomaticRelease() {
    if (gitflow.isReleaseBranch()) {
        ...
        stage('Build & Push Image') {
            ...
            dockerImage = docker.build("cloudogu/${repositoryName}-warp:${dockerReleaseVersion}", "./warp")
            docker.withRegistry('https://registry.hub.docker.com/', 'dockerHubCredentials') {
                dockerImage.push("${dockerReleaseVersion}")
            }
            dockerImage = docker.build("cloudogu/${repositoryName}-maintenance:${dockerReleaseVersion}", "./maintenance")
            docker.withRegistry('https://registry.hub.docker.com/', 'dockerHubCredentials') {
                dockerImage.push("${dockerReleaseVersion}")
            }
        }
